...
def mouseClick(events, x, y, flags, params):
    ...
    if events == cv2.EVENT_RBUTTONDOWN:
        for i, ls in enumerate(posList):
            ...
            x1, y1 = ls[0][0], ls[0][1]
            ls = pos
            if (x1 < x < x1 + 50 or x1 < x < x1 - 50) and (y1 < y < y1 + 50 or y1 < y < y1 - 50):
                posList.pop(i)

    with open('D:\MK\Coding\OJ\CarParkPos3', 'wb') as f:
        pickle.dump(posList, f)


while True:
    img = cv2.imread('D:\MK\Coding\OJ\parkinglot\carParkImg3.png')
    # print(img)
    for pos in posList:
