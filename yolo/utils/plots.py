...
class Colors:
    ...
    def __init__(self):
        # hex = matplotlib.colors.TABLEAU_COLORS.values()FFB21D
        hex = ('CFD231', 'FF9D97', 'FF701F', '48F90A', 'FF3838', '48F90A', '92CC17', '3DDB86', '1A9334', '00D4BB',
...
class Annotator:
    ...
    def box_blur(self, box, label='', color=(128, 128, 128), txt_color=(255, 255, 255)):
        # Add one xyxy box to image with label
        if self.pil or not is_ascii(label):
            self.draw.rectangle(box, width=self.lw, outline=color)  # box
            if label:
